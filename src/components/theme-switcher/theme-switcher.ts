...
export class ThemeSwitcher extends LitElement {
	...
	private _getCurrentTheme() {
		...
		if (localStorageTheme !== null) {
			...
			// Set color scheme to dark if it's the preference of the user
			if (window.matchMedia && window.matchMedia('(prefers-color-scheme: dark)').matches) {
				this._setTheme('dark');
			} else{
			// Set color to light/default if no preference, or light is specified
  ...
  firstUpdated() {
    ...
    const _heroImage = document.querySelector('#home-hero-image') as HTMLImageElement;
		if (!(_heroImage.src.endsWith("/philosophy/profile.jpg"))){
			if (theme === 'default') {
